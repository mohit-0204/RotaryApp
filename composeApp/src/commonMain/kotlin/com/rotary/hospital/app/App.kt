...
import com.rotary.hospital.feature.home.presentation.screen.SettingsScreen
...
import com.rotary.hospital.feature.opd.presentation.screen.OpdRegistrationScreen
...
@Composable
@Preview
fun App(paymentHandler: PaymentHandler? = null) {
    AppTheme {
        ...
        var localizationIso by remember { mutableStateOf(Language.English.iso) }
        val selectedLanguage by derivedStateOf {
            Language.entries.first { it.iso == localizationIso }
        }

        LaunchedEffect(Unit) {
            // Load saved language
            localizationIso =
                preferences.getString(PreferenceKeys.LANGUAGE_ISO, Language.English.iso).first()
            localization.applyLanguage(localizationIso)
        ...
        key(selectedLanguage){ // 'key' keeps track for language change detection and recomposes navHost
            NavHost(
                ...
                composable<AppRoute.Settings> {
                    SettingsScreen(
                        ...
                        onLanguageChanged = { language ->
                            ...
                            localizationIso = language.iso
                            scope.launch {
                                preferences.saveString(PreferenceKeys.LANGUAGE_ISO, language.iso)
                                Logger.d("App", "Language changed to: ${language.iso}")
                            }
                        })
                }
